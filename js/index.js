async function search(x) {
  let t = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=7d77b96c972b4d119a3151101212704&q=${x}&days=3`
  );
  ...
  if (null != t) {
    ...
    let n = `<div class="today forecast">
                <div class="forecast-header"id="today">
                    ...
                    <div class="degree"><div class="num">${
                      t.temp_c
                    }<sup>o</sup>C</div>
                    <div class="forecast-icon"><img src="https:${
                      t.condition.icon
                    }" alt="" width=90>
                    </div>
                    </div>
                    <div class="py-4">${t.condition.text}</div>
...
function displayAnother(a) {
  ...
  for (let e = 1; e < a.length; e++)
    ...
    <div class="text-info py-5">${a[e].day.condition.text}</div>
    </div>
    </div>`;
